...
export function getSeenExternalIds(source: string): Set<string> {
  const db = getDb();
  const rows = db
    .prepare("SELECT external_id FROM jobs WHERE source = ?")
    .all(source) as { external_id: string }[];
  return new Set(rows.map((r) => r.external_id));
}

export function insertJob(
  job: Omit<Job, "fetched_at"> & { fetched_at?: number },
): Job {
  const db = getDb();
...
export function listJobs(opts?: { status?: Job["status"] }): Job[] {
  ...
  if (opts?.status) {
    return db
      .prepare("SELECT * FROM jobs WHERE status = ? ORDER BY fetched_at DESC")
      .all(opts.status) as Job[];
  }
  return db
    .prepare("SELECT * FROM jobs ORDER BY fetched_at DESC")
    .all() as Job[];
}
